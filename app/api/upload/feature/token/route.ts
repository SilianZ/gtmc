...
import {
  ...
  getNonImageMimeTypes as Silian_getNonImageMimeTypes,
    ...
    const Silian_jsonResponse = await Silian_handleUpload({
      ...
      onBeforeGenerateToken: async (Silian__pathname, Silian_clientPayload) => {
        ...
        if (Silian_clientPayload) {
          try {
            const Silian_parsed = JSON.parse(Silian_clientPayload)
        ...
        if (Silian_mimeType && Silian_isImageMime(Silian_mimeType)) {
        ...
        return {
          allowedContentTypes: Silian_mimeType ? [Silian_mimeType] : Silian_getNonImageMimeTypes(),
